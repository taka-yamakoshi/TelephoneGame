...
```{r}
pd <- import("pandas")
bert_vocab <- pd$read_pickle("../data/LingFeatures/bert/21TokenSents/Lag500/VOCABFreqAll.pkl") %>% pkl_to_df() 
books_vocab <- pd$read_pickle("../data/LingFeatures/book/21TokenSents/VOCABFreqAll.pkl") %>% pkl_to_df() %>%
...
```{r}
# Note that it matters where we compute the corpus frequencies; do we restrict to just words that appear in bert or the full corpus distribution?
